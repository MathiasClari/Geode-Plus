...
import net.yeoxuhang.geode.common.block.EchoCrystalBlock;
...
public class WrappistCrystalSpikeFeature extends Feature<GeodeCrystalSpikeConfig> {
    ...
    public boolean place(FeaturePlaceContext<GeodeCrystalSpikeConfig> context) {
        ...
        if (world.isStateAtPosition(blockPos.relative(config.crystal_direction.getDirection().getOpposite()), DripstoneUtils::isEmptyOrWaterOrLava) && world.getBlockState(blockPos).is(GeodeModTagsRegistry.Blocks.CAN_LARGE_WRAPPIST_CRYSTAL_PLACE) && this.placeSpike(world, blockPos, radiusCheck, stepHeight, randomChance, trigList, config.crystal_direction.getDirection(), random)) {
        ...
        while(true) {
            ...
            for(int var11 = 0; var11 < var10; ++var11) {
                ...
                if (random.nextBoolean() && world.isStateAtPosition(relative, DripstoneUtils::isEmptyOrWater) && world.getBlockState(pos).equals(config.crystal_state)) {
                    this.setBlock(world, relative, (BlockState)((BlockState)((BlockState)config.cluster_state.setValue(EchoCrystalBlock.FACING, direction)).setValue(EchoCrystalBlock.WATERLOGGED, world.getFluidState(relative).getType() == Fluids.WATER)));
                }
            }
        }
    }
